...
use mikkle\tp_wxpay\src\SendredPack;
...
class Wxpay
{
    ...
    {
        $this->options = empty($this->options) ? $this->getOptions($options) : array_merge($this->options,
    ...
    public function SendredPack($options = [])
    {
        $options = self::getOptions($options);
        $sn      = self::getSn($options);
        if ( isset(self::$instance[$sn . "_" . __FUNCTION__]) ) {
            ...
            return self::$instance[$sn . "_" . __FUNCTION__] = new SendredPack($options);
        }
    }
